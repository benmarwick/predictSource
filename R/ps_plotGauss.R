...
#' @param ps_scatterPlot  Logical (default is TRUE): specify whether to show scatter plots when qqPlot = FALSE
...
ps_plotGauss <- function(
  ...
  {
  data1 <- data[, analyticVars[1]]
  data1 <- data1[!is.na(data1[,analyticVars[1]])]
  if (ps_qqPlot | ps_identify) {
    qqtest(data = data1, dist = "normal", drawPercentiles = T,
           main = paste(analyticVars[1],"source", groups[i_group]))
    qqnorm_pts<-qqnorm(data1, main = paste(analyticVars[1],"source", groups[i_group]))
    qqline(data1)
  }
  ...
  data2 <- data[, analyticVars[2]]
  data2 <- data2[!is.na(data[,analyticVars[2]])]
  if (qqPlot | ps_identify) {
    qqtest(data = data2, dist = "normal", drawPercentiles = T,
           main = paste(analyticVars[2],"source", groups[i_group]))
    qqnorm_pts<-qqnorm(data2, main = paste(analyticVars[2],"source", groups[i_group]))
    qqline(data2)
  }
  else if (scatterPlot) {
    qqnorm_pts<-qqnorm(data2, main = paste(analyticVars[2],"source", groups[i_group]))
  ...
  data <- data[!is.na(data[,analyticVars[1]]) & !is.na(data[,analyticVars[2]]),]
  n_samples <- nrow(data)
  ...
  browser()
  # return p-values
  if (!ps_identify) pvalues
  if ( ps_identify) list(pvalues=pvalues, dataCheck=dataCheck)
}