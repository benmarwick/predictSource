...
tree <- fn.tree(data=ObsidianSources, GroupVar="Code",Groups="All", AnalyticVars=analyticVars,
                 Model="Sr"+"Nb"+"Rb"+"Y"+"Zr", ModelTitle="Sr + Nb + Rb + Y + Zr", plotCp=F)


## ----echo=FALSE, results='hide', message=FALSE, fig.keep='last', fig.cap="Figure 1.2 Principal components plot with Jemez obsidian source convex hulls and obsidian artifacts with points outside the convex hull predicted from a classification tree model."----
data(ObsidianSources)
...
save.tree <- fn.tree(data=ObsidianSources, GroupVar="Code",Groups="All", AnalyticVars=analyticVars,
  Model = "Sr"+"Nb"+"Rb"+"Y"+"Zr", ModelTitle="Sr + Nb + Rb + Y + Zr", predictSources=T,       predictData=ObsidianArtifacts, ID="ID", plotTree=F, plotCp=F)
...
library(karon)
data(ObsidianSources)
checkData<-fn.CheckData(data=ObsidianSources,CheckDupVars=c("Code","ID"),GroupVar="Code",Groups="All",AnalyticVars=c("Sr","Rb","Nb","Y"))
knitr::kable(checkData$statistics, caption="Table 3.2: Descriptive statistics for the elements in the Jemez obsidian sources used in the classsification tree in Figure 1.1.")


## ------------------------------------------------------------------------
library(karon)
data(ObsidianArtifacts)
checkData<-fn.CheckData(data=ObsidianArtifacts,CheckDupVars=c("Code","ID"), GroupVar=" ", Groups=" ", AnalyticVars=c("Sr","Rb","Y"))
knitr::kable(checkData$statistics, caption="Table 3.3: Descriptive statistics for the elements in the obsidian artifacts used in the classsification tree in Figure 1.1.")


## ----echo=FALSE, results='hide',message=FALSE, fig.cap="Figure 4.1: Box plots for the Jemez obsidian source data."----
library(karon)
data(ObsidianSources)
...
## ----echo=FALSE, results='hide',message=FALSE, fig.cap="Figure 4.2: Box plots for rubidium and niobium comparing the Jemez obsidian sources and the obsidian artifacts assigned to those sources by Steve Shackley.  x.a labels the artifacts assigned to source x."----
library(karon)
data(ObsidianSources)
data(ObsidianArtifacts)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
...
## ----echo=FALSE, results='hide',message=FALSE, fig.cap="Figure 4.8a: Q-Q plots of rubidium and zirconium, including the bivariate Q-Q plot, for the Jemez obsidian source data from source A."----
library(karon)
data(ObsidianSources)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
plot.2d.Gauss<-fn.2dPlot.Gauss(data=ObsidianSources, GroupVar="Code", ID="ID", Groups="A",
    AnalyticVars=c("Rb","Zr"), qqPlot=T)


## ----echo=FALSE, results='hide',message=FALSE, fig.cap="Figure 4.8b: Q-Q plots of rubidium and zirconium, including the bivariate Q-Q plot, for the Jemez obsidian source data from source B."----
library(karon)
data(ObsidianSources)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
...
## ----echo=FALSE, results='hide',message=FALSE, fig.cap="Figure 4.8c: Q-Q plots of rubidium and niobium for the Jemez obsidian source data from sources A and B."----
library(karon)
data(ObsidianSources)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
...
plot.2d.Gauss<-fn.2dPlot.Gauss(data=ObsidianSources, GroupVar="Code", ID="ID", Groups="All",
    AnalyticVars=c("Rb","Zr"),qqPlot=F, scatterPlot=F)
library(karon)
data(ObsidianSources)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")    
...
## ------------------------------------------------------------------------
plot.2d.Gauss<-fn.2dPlot.Gauss(data=ObsidianSources, GroupVar="Code", ID="ID", Groups="All",
    AnalyticVars=c("Rb","Nb"),qqPlot=F, scatterPlot=F)
...
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
tree <- fn.tree(data=ObsidianSources, GroupVar="Code",Groups="All", AnalyticVars=analyticVars,
                 Model="Sr"+"Nb"+"Rb"+"Y"+"Zr", ModelTitle="Sr + Nb + Rb + Y + Zr", plotTree=F)
...
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
tree <- fn.tree(data=ObsidianSources, GroupVar="Code",Groups="All", AnalyticVars=analyticVars,
                 Model="Sr"+"Nb"+"Rb"+"Y"+"Zr", ModelTitle="Sr + Nb + Rb + Y + Zr", plotCp=F, plotTree=F)
tree$Tree
...
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
tree <- fn.tree(data=ObsidianSources, GroupVar="Code",Groups="All", AnalyticVars=analyticVars,
                 Model="Sr"+"Nb"+"Rb"+"Y"+"Zr", ModelTitle="Sr + Nb + Rb + Y + Zr", plotTree=F, plotCp = F)
...
## ---- echo=FALSE, message=FALSE, fig.cap="Figure 6.3: Principal components plot of the locations of artifact sources from classification tree model predictions."----
library(karon)
data(ObsidianSources)
data(ObsidianArtifacts)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
tree <- fn.tree(data=ObsidianSources, GroupVar="Code",Groups="All", AnalyticVars=analyticVars,
                ...
                plotTree=F, plotCp = F, predictSources=T, predictData=ObsidianArtifacts)
...
## ----echo=FALSE, results='hide',message=FALSE, fig.cap="Figure 7.3a: Box plots of the estimated probabilities for the predicted sources for the obsidian artifacts.", fig.keep='first'----
data(ObsidianSources)
data(ObsidianArtifacts)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
...
## ----echo=FALSE, results='hide',message=FALSE, fig.cap="Figure 7.3b: Box plots of the estimated probabilities of sources other than the predicted sources for the obsidian artifacts.", fig.keep='last'----
data(ObsidianSources)
data(ObsidianArtifacts)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
save.randomForest <- fn.randomForest(data=ObsidianSources, GroupVar="Code",Groups="All", AnalyticVars=analyticVars, NvarUsed=3, plotErrorRate=F, plotImportance=F, predictSources=T,predictData=ObsidianArtifacts, plotSourceProbs=T)
...
## ----echo=FALSE, results='hide',message=FALSE, fig.cap="Figure 8.1: Box plots for ytterbium and niobium comparing the Jemez obsidian sources and the obsidian artifacts assigned to those sources by a clasification tree analysis.  The plot for x.a is the artifacts assigned to source x."----
library(karon)
data(ObsidianSources)
data(ObsidianArtifacts)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
...
boxPlots<-fn.BoxPlots(data=SourcesArtifacts, GroupVar="Source", Groups="All",AnalyticVars=c("Y","Nb"),Nrow=2,Ncol=1)


## ----echo=FALSE, results='hide', message=FALSE, fig.keep='first', fig.cap="Figure 8.2a: Principal components plots of the Jemez obsidian source convex hulls with the source points (left panel) and obsidian artifact points (right panel).  The artifacts are labelled with their predicted source from a classification tree analysis."----
data(ObsidianSources)
data(ObsidianArtifacts)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
sources <- unique(ObsidianSources[,"Code"])
save.tree <- fn.tree(data=ObsidianSources, GroupVar="Code",Groups="All", AnalyticVars=analyticVars,
  Model = "Sr"+"Nb"+"Rb"+"Y"+"Zr", ModelTitle="Sr + Nb + Rb + Y + Zr", predictSources=T, predictData=ObsidianArtifacts, ID="ID",
  plotTree=F, plotCp=F)
pca.eval <- fn.pca.evaluation(SourceData=ObsidianSources, ArtifactData=save.tree$predictedSources,
  SourceGroup= "Code", ArtifactGroup="source",known.sources=sources, predicted.sources=sources,
  AnalyticVars=analyticVars, plotAllPoints=T, plotHullsOutsidePoints=T, plotOutsidePoints=F)
  

## ----echo=FALSE, results='hide', message=FALSE, fig.keep='last', fig.cap="Figure 8.2b: Principal components plots of the Jemez obsidian source convex hulls with the source point medians (left panel) and obsidian artifact points outside the predicted source hulls (right panel).  The artifacts are labelled with their predicted source from a classification tree analysis."----
data(ObsidianSources)
data(ObsidianArtifacts)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
sources <- unique(ObsidianSources[,"Code"])
...
## ----echo=FALSE, results='hide', message=FALSE, fig.keep='last', fig.cap="Figure 8.2c: Principal components plot with Jemez obsidian source convex hulls and obsidian artifacts with points outside the convex hull predicted from a classification tree model."----
data(ObsidianSources)
data(ObsidianArtifacts)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
sources <- unique(ObsidianSources[,"Code"])
...
knitr::kable(tree.data.check, caption="Table 8.1: Artifacts which may have misidentified sources identified by using Identify=T in fn.pca.evaluation() after fitting a tree model.  The variable group is the predicted source from the model.")


## ----echo=FALSE, results='hide', message=FALSE, fig.keep='last', fig.cap="Figure 8.3: Principal components plot of the Jemez obsidian source convex hulls and obsidian artifacts with points outside the convex hull predicted from a random forests analysis."----
data(ObsidianSources)
data(ObsidianArtifacts)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
sources <- unique(ObsidianSources[,"Code"])
...
## ----echo=FALSE, results='hide', message=FALSE, fig.keep='last', fig.cap="Figure 8.4: Principal components plot with Jemez obsidian source convex hulls and obsidian artifacts with points outside the convex hull labelled with Steve Shackley's source predictions."----
data(ObsidianSources)
data(ObsidianArtifacts)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
sources <- unique(ObsidianSources[,"Code"])
...
save.randomForest <- fn.randomForest(data=ObsidianSources, GroupVar="Code",Groups="All",
    AnalyticVars=analyticVars, ID="ID", NvarUsed=3, plotErrorRate=F, plotImportance=F,
    predictSources=T, predictData=ObsidianArtifacts, plotSourceProbs=F)
tree.data.check <- tree.data.check[order(tree.data.check[,"ID"]),]
save.rF<-save.randomForest$predictedSources[order(save.randomForest$predictedSources[,"ID"]),]
obsCheck<-merge(x=save.rF, y = tree.data.check[,c("ID","pc1","pc2")],
                by = "ID", x.all=F, y.all=F)
knitr::kable(obsCheck, caption="Table 8.2: Random forest probability estimates for artifacts which may have  been misidentified as from source C using the tree model identified by using Identify=T in fn.pca.evaluation().  source is the predicted source from the random forest procedure.")
