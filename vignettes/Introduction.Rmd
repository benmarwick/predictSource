...
"Rb"+"Sr"+"Y"+"Zr"+"Nb".  Note that one artifact predicted to be from source B is closer to the hull from source C, and perhaps eight artifacts predicted to be from source C are close to or closer to the hull from source B.  Qualitatively, this agrees with the potential for misclassification from the random forest analysis in Table z3.

In contrast, in the corresponding plot based on the random forest analysis, these nine artifacts are predicted to come from the sources generating the convex hulls to which they are closest.  We encourage the reader to run this code (contained in the documentation for the function) to verify this statement. 
