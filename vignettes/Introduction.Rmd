...
```{r, echo=FALSE, message=FALSE, fig.cap="Figure 6.3: Principal components plot of the locations of artifact sources as predicted by the classification tree model."  }
library(karon)
data(ObsidianSources)
data(ObsidianArtifacts)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
...
```{r  echo=FALSE, results='hide',message=FALSE, fig.cap="Figure 7.3:"}
data(ObsidianSources)
data(ObsidianArtifacts)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
...
pca.eval <- fn.pca.evaluation(SourceData=ObsidianSources, ArtifactData=save.tree$predictedSources,
  ...
  AnalyticVars=analyticVars, plotAllPoints=T, plotHullsOutsidePoints=T, plotOutsidePoints=T)
```
Table 8.1 contains the data generating these nine points.  These are the same artifacts identified as having a potential for misclassification from the random forest analyis (Table xx).
...
In contrast, in the corresponding plot based on the random forest analysis, these nine artifacts are predicted to come from the sources generating the convex hulls to which they are closest: see the plots below.   
