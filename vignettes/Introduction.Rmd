...
save.tree <- fn.tree(data=ObsidianSources, GroupVar="Code",Groups="All", AnalyticVars=analyticVars,
  Model = "Sr"+"Nb"+"Rb"+"Y"+"Zr", predictSources=T, predictData=ObsidianArtifacts, ID="ID",
  plotTree=F, plotCp=F)
...
library(karon)
data(ObsidianSources)
CheckData<-fn.CheckData(data=ObsidianSources,CheckDupVars=c("Code","ID"),GroupVar="Code",Groups="All",AnalyticVars=c("Sr","Rb","Y"))
...
Note that one artifact assigned to source D has values suggesting that it is actually from source C, and that one assigned to source B has values that suggest it is from another source (the rubidium value suggests source D).  These plots are also useful in evaluating the accuracy of source predictions.
...
```{r  echo=FALSE, results='hide', message=FALSE, fig.keep='last', fig.cap="Figure 8.2b: Principal components plots of the Jemez obsidian source convex hulls with the source point medians (left panel) and obsidian artifact points outside the predicted source hulls (right panel)."}
data(ObsidianSources)
data(ObsidianArtifacts)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
sources <- unique(ObsidianSources[,"Code"])
...
Eleven or twelve artifacts predicted to be from source C are close to or closer to the hull from source B in Figure 8.1c (there appears to be one such point at the upper right corner of the hull for source B).  Table 8.1 contains the data generating eleven such points ("Group" is the predicted source group).  [Compare to random forest analysis.]
...
knitr::kable(obsCheck, caption="Table 8.2: Random forest probability estimates for artifacts which may have misidentified sources identified by using Identify=T in fn.pca.evaluation() after fitting a tree model.")
...
We note that the principal components evaluation plot is useful in verifying the validity of assignments of artifacts to sources.  This evaluation identified two artifacts for which Shakely’s assignment was wrong; one of these is the artifact with ID xxxxx identified in Section x.x.
