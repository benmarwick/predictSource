...
We suggest two analyses for evaluating whether predicted sources for artifacts are correct: box plots similar to Figure 4.2 (Figure 8.1), and principal components plots. It is clear from the plots for niobium in Figure 8.1 that some artifacts are improperly assigned to source C, and perhaps one artifact is improperly assigned to source B.  The function generating principal components plots will allow us to identify the data generating these suspect assignments.  

```{r echo=FALSE, results='hide',message=FALSE, fig.cap="Figure 8.1: Box plots for rubidium and niobium comparing the Jemez obsidian sources and the obsidian artifacts assigned to those sources by a clasification tree analysis.  The plot for x.a is the artifacts assigned to source x." } 
library(karon)
data(ObsidianSources)
data(ObsidianArtifacts)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
...
boxPlots<-fn.BoxPlots(data=SourcesArtifacts, GroupVar="Source", Groups="All",AnalyticVars=c("Rb","Nb"),Nrow=2,Ncol=1)
...
Eleven or twelve artifacts predicted to be from source C are close to or closer to the hull from source B in Figure 8.1c (there appears to be one such point at the upper right corner of the hull for source B).  Table 8.1 contains the data generating eleven such points.  
...
Table 8.2 contains the source assignment probability estimates from the random forests analysis for the artifacts tabulated in Table 8.1, all of which were assigned to source C by the classification tree analysis.  The first three artifacts are correctly identified as from source C, while the remaining ones should be assigned to source B (this is also clear by comparing the ytterbium and niobium values to those for the sources in Table 3.2). 
...
obsCheck<-merge(x=save.rF, y = tree.data.check, by = "ID", x.all=F, y.all=F)
knitr::kable(obsCheck, caption="Table 8.2: Random forest probability estimates for artifacts which may have  been misidentified as from source C identified by using Identify=T in fn.pca.evaluation() after fitting a tree model.")
