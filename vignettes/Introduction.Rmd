...
The second plot shows that there are a few relatively large probabilities for unknowns incorrectly assigned to sources B and C.  Table z3 displays the data from the data frame predictedSources yielding the probabilities between 0.19 and 0.30 (note that a single point on the plot can represent more than one unknown).  The unknown yielding the large probability in Figure z2 for source B was predicted to be from source C; the unknowns yielding the large probabilties for source C were predicted to be from source B.  This suggests that it could be somewhat difficult to distinguish between artifacts from sources B and C.  Figure z3 shows a principal components plot (using fn.pca()) of the artifacts predicted to be from sources B, C, and D (the artifact from A was eliminated, as its point is far from the others).  Artifacts predicted to be from sources B and C appear to be well separated, so additional analysis would be necessary if potential misclassification between B and C were of concern.
