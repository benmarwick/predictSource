...
The plot below shows the relations between elements at source A.  It shows that, at source A, the relationships between rubidium and ytterbium, and between rubidium and zirconium, are roughly linear, but that, roughly, there is no relation between the values of strontium and those of ytterbium and zirconium.  Note that it is necessary to look at both scatterplots of the relation between a pair of variables, as limited variation in one variable and dependence of the location of the largest and smallest values of one variable can make there seem to be a relationship (see the plots of strontium as the ordinate versus ytterbium or zirconium).  Numerical estimates of correlations can confirm these observations (see the next section).
...
Numerical estimates of the associations between pairs of analytic variables, in the form of Spearman correlation coefficients, are produced by the function fn.CV.corr().  This function also produces an estimate of the coefficient of variation (the standard deviation divided by the mean) for each variable.  The Spearman correlation coefficient is computed from the ranks of the data; therefore, it is not affected by the numerical value of an outlier.  The coefficient of variation does use the numerical values, hence is affected by an outlier.  This function returns a list with elements with tne names CV (the coefficients of variation) and corr (the correlations).  By default, both the coefficients of variation and the correlations are rounded to two decimal places.    
...
The table below contains the correlation coefficient estimates for all pairs of variables at each source.  These estimates confirm the strength of the relationships suggested by the pairs plots for source A: the relations between rubidium and both ytterbium and zirconium (correlations of at least 0.58) are much stronger than the relationship betwee rubidium and strontium (correlation of 0.26), and the lack of a relation between stontium and ytterbium (correlation estimate, 0.16).  However, the estimated correlation between strontium and zirconium is 0.51, suggesting a relation between these elements at source A, in contrast to our impression from the pairs plot.  The correlation table is very useful for summarizing the relationships between pairs of elements at the sources: it is clear that those relationships vary substantially among the sources.
...
```
The following table shows the coefficients of variation for each element at each source.  Each standard deviation is small compared to the corresponding mean (at most 0.11, and most are at 0.05 or less) except for strontium at sources C and D.  The table of descriptive statistics (not shown) gives mean values for strontium at these sources as 5 and 10, respectively, with ranges of 0 to 8 and 2 to 11, respectively.
