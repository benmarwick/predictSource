...
save.randomForest <- fn.randomForest(data=ObsidianSources, GroupVar="Code",Groups="All",
    AnalyticVars=analyticVars, ID="ID", NvarUsed=3, plotErrorRate=F, plotImportance=F,
    predictSources=T, predictData=ObsidianArtifacts, plotSourceProbs=F)
index<-save.randomForest$predictedSources[,"ID"] %in% tree.data.check[,"ID"]
obsCheck<-save.randomForest$predictedSources[index,]
knitr::kable(obsCheck[,-1], caption="Table 8.2: Random forest probability estimates for artifacts which may have  been misidentified as from source C identified by using Identify=T in fn.pca.evaluation() after fitting a tree model.")
