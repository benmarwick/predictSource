...
```{r echo=FALSE, results='hide',message=FALSE, fig.cap="Figure 4.5: Example of scatterplots for the obsidian Jemez source data with confidence ellipses and robust lowess smoothing lines."} 
library(karon)
data(ObsidianSources)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
plot_2d <- ps_2dPlot(data = ObsidianSources, GroupVar = "Code", ID = "ID", Groups = c("A","B","C","D"),
                AnalyticVars = analyticVars, VariablePairs=rbind(analyticVars[c(1,4)],analyticVars[c(1,5)]), PlotEllipses=TRUE, LowessLine=TRUE, ByGroup=TRUE)
```
...
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
plot_2d <- ps_2dPlot(data = ObsidianSources, GroupVar = "Code", ID = "ID", Groups = c("A", "B", "C", "D"),
          AnalyticVars = analyticVars, VariablePair=rbind(analyticVars[c(1,4)], analyticVars[c(1,5)]), PlotEllipses=TRUE, KernelSmooth=TRUE, LowessLine=FALSE)
```
...
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
plot_2d <- ps_2dPlot(data = ObsidianSources, GroupVar = "Code", ID = "ID", Groups = "All",
          AnalyticVars = analyticVars, VariablePairs=analyticVars[c(1,4)], ByGroup = FALSE, PlotEllipses=TRUE, LowessLine=TRUE)
```
...
```{r echo=FALSE, results='hide',message=FALSE, fig.cap="Figure 8.1: Box plots for ytterbium and niobium comparing the Jemez obsidian sources and the obsidian artifacts assigned to those sources by a classification tree analysis.  The plot for x.a is the artifacts assigned to source x." } 
library(karon)
data(ObsidianSources)
data(ObsidianArtifacts)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
...
```{r  echo=FALSE, results='hide', message=FALSE, fig.keep='last', fig.cap="Figure 8.4: Principal components plot with Jemez obsidian source convex hulls and obsidian artifacts with points outside the convex hull labeled with Steve Shackley's source predictions."}
data(ObsidianSources)
data(ObsidianArtifacts)
analyticVars<-c("Rb","Sr","Y","Zr","Nb")
sources <- unique(ObsidianSources[,"Code"])
