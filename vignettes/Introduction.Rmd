...
The principal component plots of the predictions from the tree and random forest analysis demonstrate the utility of such a plot in evaluating predictions.  The function fn.pca.evaluation() add these points to a principal component plot with the convex hulls of the source data.  For accurate predictions, the points generated by the unknowns predicted to come from a source should fall within or close to the convex hull of that source.  Defining the parameter Identify to be T allows the user to create a data frame containing the data generating points that may represent misclassified unknowns.