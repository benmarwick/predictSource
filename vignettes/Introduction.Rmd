...
knitr::kable(plot.2d.Gauss$pvalues,caption="Table 7.b.  P-values from test statistics for a bivariable Rb/Nb distribution.")
#
```
...
A classification tree is obtained from recursive partitioning when the target (dependent) variable is categorical (a regression tree has a continuous dependent variable).  A tree model does a succession of binary splits, so that the result is similar to a botanical key (when there are only two subgroups at each decision point).  The classification tree procedure has a number of desirable properties:
it is nonparametric (data need not be transformed before being offered to the model, and outliers to not affect the results);
...
The function fn.tree() implements the classification tree procedure.

The tree procedure starts by finding the variable in the model statement and the two subgroups defined using that variable that yields two subgroups of the outcome variable (in the case of the obsidian data, subgroups of the sources) that are as "pure" as possible.  The default criterion, used in fn.tree(), is the Gini criterion. 

The tree procedure is implemented using a model statement containing the predictor variables separated by "+" signs.  It is important to note that the variables are offered to the model in the order in which they appear in the model statement.  If several predictor variables yield equally good splits at a node, the procedure uses the variable that appears first in the model statement.  Therefore, model statements with variables in different orders can yield different categorical trees.  As a result, we suggest running a random forest analysis before a classification tree model, then list the predictor variables in the tree model statement in the order of importance estimated from the random forest model.  
